...
POST /generate (billable endpoint)
	...
	|     +-- store usage_event in PostgreSQL
	|
	+-- QuotaService.check(tenant, type, qty)
				+-- under limit -> allow
				+-- over limit -> 429 / 402 + clear message
...
cp .env.example .env
# Keep the local Postgres port on 5433
docker compose up -d
npm install
...
Send `Idempotency-Key: <unique-uuid>` with POST /generate.
The same key and same request create one usage event only. Retries are safe.
...
cost = (input_tokens × $0.000003)
		 + (cached_input_tokens × $0.0000015)
		 + (output_tokens × $0.000015)
		 + (reasoning_tokens × $0.000015)
```
